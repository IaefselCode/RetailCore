...
export const AdaptiveSlider: FC<AdaptiveSliderProps> = ({
  ...
  const dots = useMemo(
    () =>
      Array.from({ length: 6 }).map((_, i) => (
        <div
          key={i}
          className="z-30 h-1.5 w-1.5 rounded-full bg-muted-foreground/40 transition-colors"
          style={{ opacity: 0.8 }}
        />
      )),
    []
  ...
  return (
    <motion.div className="flex h-[60vh] w-xs flex-col items-center justify-center rounded-[36px] bg-card p-6 shadow-2xl shadow-black/5 transition-colors select-none sm:w-sm sm:p-12">
      ...
      <div className="group relative flex h-13 w-full items-center overflow-hidden rounded-full bg-muted transition-colors">
        ...
        <motion.div
          ...
          transition={{ type: "spring", stiffness: 300, damping: 30 }}
        >
          <div className="size-10 rounded-full bg-background shadow-[inset_0_2px_4px_rgba(0,0,0,0.06)]" />
        </motion.div>
      </div>
    </motion.div>
  );
